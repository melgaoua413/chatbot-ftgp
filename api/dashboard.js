...
module.exports = async function handler(req, res) {
  ...
  if (req.query.json === "1") {
    try {
      ...
      var keywords = {};
      var stop = ["le","la","les","de","du","des","un","une","est","que","qui","quoi","comment","je","tu","il","nous","vous","ils","et","ou","mais","donc","car","pas","plus","tres","bien","aussi","pour","avec","dans","sur","par","au","aux","en","ce","mon","ton","son","ma","ta","sa"];
      if (qRaw.data) qRaw.data.forEach(function(r){ if(!r.question)return; r.question.toLowerCase().split(/\s+/).forEach(function(w){ w=w.replace(/[^a-zA-ZÀ-ÿ]/g,""); if(w.length>3&&!stop.includes(w)) keywords[w]=(keywords[w]||0)+1; }); });
...
.box{background:white;border-radius:16px;padding:24px;box-shadow:0 2px 12px rgba(22,11,71,.08)}
...
.day-chart{display:flex;align-items:flex-end;gap:6px;height:100px;margin-top:8px}
...
.day-val{font-size:10px;color:#0045B3;font-weight:600}
.recent-item{padding:12px 0;border-bottom:1px solid #EEF2FF}
...
<script>
function ago(iso){var d=new Date(iso),n=new Date(),m=Math.floor((n-d)/60000);if(m<1)return"à l'instant";if(m<60)return m+"min";if(m<1440)return Math.floor(m/60)+"h";return Math.floor(m/1440)+"j";}
  ...
  .then(function(d){
    ...
    var html=
      '<div class="cards">'+
        ...
        '<div class="card"><div class="label">Thèmes détectés</div><div class="value">'+(d.topKeywords?d.topKeywords.length:0)+'</div><div class="sub">Mots-clés uniques</div></div>'+
      '</div>'+
      '<div class="grid2">'+
        '<div class="box"><h2>📅 Activité 7 derniers jours</h2><div class="day-chart">'+
          days.map(function(e){var h=Math.max(4,Math.round((e[1]/maxD)*88));return'<div class="day-col"><div class="day-val">'+e[1]+'</div><div class="day-bar" style="height:'+h+'px"></div><div class="day-label">'+fdate(e[0])+'</div></div>';}).join("")+
        '</div></div>'+
      ...
      '<div class="box"><h2>💬 Dernières conversations</h2>'+
        (d.recent||[]).map(function(r){return'<div class="recent-item"><div class="recent-q">'+r.question+'</div><div class="recent-a">'+r.answer+'</div><div class="recent-time">'+ago(r.created_at)+'</div></div>';}).join("")+
      '</div>';
    document.getElementById("app").innerHTML=html;
  })
  .catch(function(){document.getElementById("app").innerHTML='<div style="text-align:center;padding:40px;color:#EF4444">Erreur de chargement</div>';});
}
load();
</script>
</body>
