...
// Clé secrète pour protéger le webhook (à ajouter aussi dans Vercel env vars)
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!body.airtable_id || !body.nom_commercial) {
      return res.status(400).json({ error: "Champs obligatoires manquants (airtable_id, nom_commercial)" });
    }

    var sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY || process.env.SUPABASE_KEY);

    // Upsert : insère ou met à jour si l'airtable_id existe déjà
    var { data, error } = await sb
      ...
      .upsert({
        ...
        annee_creation:   body.annee_creation || null,
        secteurs_cibles:  body.secteurs_cibles || [],
        type_solution:    body.type_solution || [],
        briques_ia:       body.briques_ia || [],
        fonctions_metier: body.fonctions_metier || [],
        labels:           body.labels || [],
